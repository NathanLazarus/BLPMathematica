...
T = 40;                                             % # Markets
starts = 1; % XXX 5;                                         % # random start values to check during estimation
...
%            'GradConstr','on','GradObj','on','TolCon',1E-6,'TolFun',1E-6,'TolX',1E-6);  % ,'JacobPattern',ConsPattern,'Hessian','user-supplied','HessFcn',@GMMMPEC_hess_ktr, 'HessPattern', HessianPattern);  
    ktropts = knitro_options('algorithm',1,'outlev',4, ...
                     ...
                     'derivcheck', 1, 'derivcheck_tol', 1E-5);
                    
    extendedFeatures = {};
    extendedFeatures.JacobPattern = ConsPattern;
    extendedFeatures.HessPattern = HessianPattern;
    extendedFeatures.HessFcn = @GMMMPEC_hess_ktr;
      
      
    t1 = cputime;   
       
    [X,fval_rep,exitflag,output,lambda] = knitro_nlp(@GMMMPEC_f_ktr, x0, [], [], [], [], x_L, x_U, @GMMMPEC_c_ktr, extendedFeatures, ktropts, 'knitroOptions2.opt') ; % XXX BSS
    
    % [X fval_rep exitflag output lambda] = knitro_nlp(@GMMMPEC_f_ktr,x0,[],[],[],[],x_L,x_U,@GMMMPEC_c_ktr, extendedFeatures, ktropts, 'knitroOptions2.opt');    
