...
global iteration K T prods IV tol_outer share numProdsTotal
...
%     neF    = numProdsTotal + size(IV, 2);
%     c_L = zeros(neF,1);
%     c_U = zeros(neF,1);
    
%     c_L(1:numProdsTotal) = log(share);
%     c_U(1:numProdsTotal) = log(share);
    
%     c_L(1:numProdsTotal) = share;
%     c_U(1:numProdsTotal) = share;
    
   
    nx0 = size(x00,1);
    HessPattern = zeros(nx0,nx0);
    HessPattern(2*K+3+numProdsTotal:end,2*K+3+numProdsTotal:end) = 1;       % specify sparsity pattern for Hessian
    
    nIV = size(IV,2);
    
    c11 = zeros(numProdsTotal, K+1);
    c12 = ones(numProdsTotal, K+1);
    c13 = repmat(eye(T), prods, prods);
    c14 = zeros(numProdsTotal, nIV);
    
    c21 = ones(nIV, K+1);    
    c22 = zeros(nIV, K+1);    
    c23 = ones(nIV, numProdsTotal);
    c24 = eye(nIV);     
