...
% --------------------
% 16dec2020 bss udpated for latest KNITRO and to dynamically determine optimum.
...
% %   nQuadType, nSolMCDraws ) ;
  ...
  N_MC_INT         = 100 ;          % How many different MC draws to try for integral: is this the number of replications?
  ...
  % Find and use best point estimate
  ixValid = mExitCode == 0 | mExitCode == 1 ; % Valid solution
  [fBest, ix_min] = min(mFOpt(ixValid)) ;     % Find best valid optimum
  ixBest = find(mFOpt == fBest, 1) ;          % Take first optimal solution which matches smallest fval at optimum
  xOpt = mXOpt( :, ixBest ) ;
  fprintf( 1, 'Using best start to compare shares: %4d with EXIT=%d fval=%d\n', ixBest, mExitCode(ixBest), mFOpt(ixBest));
...
%     vGHNodes = [ 7 ] ;
  ...
  mSharesSim = zeros( numProdsTotal, N_MC_INT, nParamValues ) ;
    
  mSharesNied = zeros( numProdsTotal, N_MC_INT, nParamValues ) ;
    
  mSharesGH  = zeros( numProdsTotal, nGHTypes, nParamValues ) ;
  mSharesStroud = zeros( numProdsTotal, 2, nParamValues ) ;
% %     mResidStroud  = zeros( numProdsTotal, 2, nParamValues ) ;
  mSharesSparse = zeros( numProdsTotal, 1, nParamValues ) ; 
  ...
  % xOpt( 2 * nTheta + ( 1 : numProdsTotal ) )  - delta ( [ -price  x ]'theta1 + xi_jt )
  % xOpt( 2 * nTheta + numProdsTotal + ( 1 : numProdsTotal ) )  - g = Z'xi_jt
  ...
  % %     mSharesSim = zeros( numProdsTotal, N_MC_INT, nParamValues ) ;
  % %     mResidSim  = zeros( numProdsTotal, N_MC_INT ) ;
  % %     vIter      = zeros( N_MC_INT ) ;      % number of iterations for Berry's mapping to converge
      ...
      for ix = 1 : N_MC_INT
        ...
        mSharesSim( :, ix, ixParam ) = ComputeShares( xParam ) ;   % Market shares
...
% %         [ mResidSim( :, ix ), vIter( ix ) ]  = ComputeShock( xParam ) ;    % Compute xi_jt
  ...
  % %     mSharesGH  = zeros( numProdsTotal, nGHTypes, nParamValues ) ;
      ...
      for jx = 1 : nGHTypes
        ...
        GHQuadInit( length(betatrue), vGHNodes( jx ) ) ;  % Change the last argument to change the number of nodes
...
% %   set( 222, 'Visible', 'off' ) ;
  ...
  vIxSingleVec  = ( jxParam - 1 ) * numProdsTotal + ( 1 : numProdsTotal )' ;
  vIxStroud     = ( jxParam - 1 ) * numProdsTotal * size( mSharesStroud, 2 ) + ( 1 : numProdsTotal )' ;
  
  nGHOffset     = ( find( 7 == vGHNodes ) - 1 ) * numProdsTotal ;               % Offset to get to the 7^5 rule
  vIxGH         = ( jxParam - 1 ) * numProdsTotal * size( mSharesGH, 2 ) + ( 1 : numProdsTotal )'         ...
                    + nGHOffset ;
  vIxSingleMat  = ( jxParam - 1 ) * numProdsTotal * N_MC_INT + ( 1 : numProdsTotal * N_MC_INT )' ;
...
% %   hold on ;
  ...
  plot( mSharesStroud( vIxStroud + numProdsTotal ), vStdDev, 'b>' ) ;   % Right Col of Stroud
...
% %   hold on ;
  ...
  plot( log10( mSharesStroud( vIxStroud + numProdsTotal ) ), vLogStdDev, 'b>' ) ;   % Right Col of Stroud
...
% %   set( 400, 'Visible', bVisible ) ;
  ...
  plot( vBins, cumsum( mCount ) ./ ( numProdsTotal ), '.' ) ;
  ...
  vDelta = xOpt( 2 * K + 3 : 2 * K + 2 + numProdsTotal, 1 ) ;         % mean utilities
