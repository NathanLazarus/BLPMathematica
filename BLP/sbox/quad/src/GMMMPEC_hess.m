...
for t=1:T,
    ...
    for rr = 1:nn,
        ...
        dL2ddelta2DIAG(index,1:prodsMarket(t)) = dL2ddelta2DIAG(index,1:prodsMarket(t)) + blk * Q_WEIGHTS(rr);
        ...
        dL2ddeltadtheta(index,:) = dL2ddeltadtheta(index,:) + dL2ddeltadthetarr * Q_WEIGHTS(rr);
        dL2dtheta22rr = ((multip*ooo).*dSdtheta2rr)'*xsimSxv + sumprod_mpsimS*(-dSdtheta2rr'*x(index,:).*(ones(K+1,1)*v(:,rr)')) ;
        dL2dtheta22 = dL2dtheta22 + dL2dtheta22rr * Q_WEIGHTS(rr);
